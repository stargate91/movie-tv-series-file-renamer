...
def get_codec(file_path):
    ...
    try:
        ...
        logger.error(f"Error while probing codec for {file_path}: {e}")
        return "unknown"
...
def get_audio_codec(file_path):
    ...
    try:
        ...
        logger.error(f"Error reading audio codec for {file_path}: {e}")
        return "unknown"
...
def get_video_bitrate(file_path):
    ...
    try:
        ...
        logger.error(f"Error reading video bitrate for {file_path}: {e}")
        return "unknown"
...
def get_framerate(file_path):
    ...
    try:
        ...
        logger.error(f"Error getting framerate for {file_path}: {e}")
        return "unknown"
...
def get_audio_channels(file_path):
    ...
    try:
        ...
        logger.error(f"Error reading audio channels for {file_path}: {e}")
        return "unknown"
...
def get_first_audio_language_code(file_path):
    ...
    try:
        ...
        logger.error(f"Error reading audio language for {file_path}: {e}")
        return "unknown"
...
def get_hdr_info(file_path):
    file_path = str(Path(file_path).resolve())
    try:
        probe = ffmpeg.probe(file_path, select_streams='v:0', show_entries='stream=color_space,color_transfer,color_primaries', v='error')
        stream = probe['streams'][0]
        transfer = stream.get('color_transfer', '')
        primaries = stream.get('color_primaries', '')
        
        if 'smpte2084' in transfer or 'bt2020' in primaries:
            return "HDR"
...
def get_bit_depth(file_path):
    file_path = str(Path(file_path).resolve())
    try:
        probe = ffmpeg.probe(file_path, select_streams='v:0', show_entries='stream=pix_fmt', v='error')
        pix_fmt = probe['streams'][0].get('pix_fmt', '')
        if '10p' in pix_fmt: return "10bit"
        if '12p' in pix_fmt: return "12bit"
        return "8bit"
    except:
        return "8bit"

def get_subtitle_languages(file_path):
    file_path = str(Path(file_path).resolve())
    try:
        probe = ffmpeg.probe(file_path, select_streams='s', show_entries='stream_tags=language', v='error')
        langs = []
        for stream in probe.get('streams', []):
            lang = stream.get('tags', {}).get('language')
            if lang:
                code = get_language_code_2(lang)
                if code != "unknown" and code not in langs:
                    langs.append(code)
...
def get_audio_stream_count(file_path):
    file_path = str(Path(file_path).resolve())
    try:
        probe = ffmpeg.probe(file_path, select_streams='a', v='error')
...
def get_video_metadata(file_path):
    ...
    # New Fields
    metadata['hdr_type'] = get_hdr_info(file_path)
    metadata['bit_depth'] = get_bit_depth(file_path)
    metadata['subtitle_languages'] = get_subtitle_languages(file_path)
    metadata['audio_streams_count'] = get_audio_stream_count(file_path)
