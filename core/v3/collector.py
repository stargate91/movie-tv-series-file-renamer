...
class Collector:
    ...
    def _phase_nfo(self, cb=None):
        """Parses .nfo files to extract IMDB IDs, then writes them to the parent video."""
        nfo_files = self.db.get_files_by_category('metadata')
        
        for i, nfo in enumerate(nfo_files):
            if cb: cb(f"NFO: {nfo['file_name']}", i, len(nfo_files))
    ...
    def _phase_ffmpeg(self, cb=None):
        """Probes all video and audio files with ffprobe concurrently."""
        import concurrent.futures
        
        media_files = self.db.get_files_by_category('video', 'audio')
        completed = 0
    ...
    def _phase_guessit(self, cb=None):
        """Parses filename and foldername with GuessIt for non-NFO files concurrently."""
        from guessit import guessit
        import concurrent.futures
        import threading
        
        media_files = self.db.get_files_by_category('video', 'audio', 'subtitle')
        completed = 0
        total = len(media_files)
